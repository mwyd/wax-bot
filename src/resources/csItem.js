...
const rareDopplerPhases = ["Emerald", "Ruby", "Sapphire", "Black Pearl"];
...
const getShortName = (hashName) => {
  return hashName.replace("StatTrak™ ", "").split("(")[0].trim();
};
...
export const updateItemDetails = async (item) => {
  ...
  item.$conduit_hash_name = item.$phase
    ? item.name.replace("(", `${item.$phase} (`)
    : item.name;
  ...
  if (paintseed !== null && hasPaintSeedVariant(item.name)) {
    const { success, data } = await csItem.getVariant(
      session.token,
...
export const getFloatRarity = (float) => {
  ...
  return float <= highRankFloat ? csItemDetailRarityEnum.RARE : rarity;
};
